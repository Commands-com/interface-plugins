...
import { computeInterfacePluginSha256 } from './compute-interface-plugin-sha256.mjs';
...
async function main() {
  ...
  if (!pluginRoot || !outputPath) {
    console.error('Usage: node generate-interface-allowlist.mjs [--managed-only] <pluginsDir> <outputFile>');
    process.exit(1);
  }
  ...
  for (const entry of entries) {
    ...
    if (managedOnly) {
      const markerPath = path.join(pluginPath, '.installed-by-commands-interface-plugins');
      try {
  ...
  for (const name of pluginDirs) {
    ...
    const digest = await computeInterfacePluginSha256(pluginPath);
